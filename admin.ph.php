    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_POST['add_movie'])) {
            ...
            if (empty($title) || empty($trailer_url) || empty($classification)) {
                $error = "Título, URL del tráiler y Clasificación son obligatorios.";
                ...
                $description = !empty($tmdb_data['description']) ? $tmdb_data['description'] : $description;
                $duration = !empty($tmdb_data['runtime']) ? intval($tmdb_data['runtime']) : $duration;
                $genre = !empty($tmdb_data['genres']) ? $tmdb_data['genres'] : $genre;
                $director = !empty($tmdb_data['director']) && $tmdb_data['director'] !== 'No disponible' ? $tmdb_data['director'] : $director;
                $cast_members = !empty($tmdb_data['cast_members']) ? $tmdb_data['cast_members'] : $cast_members;
            ...
            if ($id <= 0) {
                ...
                try {
                    ...
                    if (!empty($fields_updated)) {
                        ...
                        $msg_text = "Película actualizada exitosamente.";
                    }
        ...
        elseif (isset($_POST['edit_user'])) {
            ...
            if ($user_id <= 0) {
                ...
                $error = "El nombre debe tener entre 2 y 100 caracteres.";
            } elseif (!empty($cedula_number) && strlen($cedula_number) > 20) {
                $error = "El número de cédula no puede exceder los 20 caracteres.";
            } elseif (!empty($phone_number) && strlen($phone_number) > 20) {
                $error = "El número de teléfono no puede exceder los 20 caracteres.";
            } else {
        ...
        // -----------------------------------------
        // CONFIGURACIÓN DEL SITIO - AGREGAR IVA
        // -----------------------------------------
...
<body class="bg-gray-900 text-white min-h-screen p-4 md:p-8">
    <div class="max-w-7xl mx-auto">
        ...
        <!-- ============================================ -->
        <!-- TAB: HORARIOS - MODIFICADO CON PRECIOS       -->
        <!-- ============================================ -->
        ...
        <!-- ============================================ -->
        <!-- TAB: CONFIGURACIÓN - MODIFICADO CON IVA      -->
        <!-- ============================================ -->
