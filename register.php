        ...
        if (
            ...
            try {
                $stmt_check_cedula = $pdo->prepare("
                    SELECT id FROM users WHERE cedula_type = ? AND cedula_number = ?
                ");
...
<body class="bg-gray-900 text-white min-h-screen flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-gray-800 rounded-lg shadow-xl p-8 border border-gray-700 my-8">
        ...
        <div class="text-center mb-6">
            <?php if ($hasLogo): ?>
                <img src="<?= htmlspecialchars($logoSrc) ?>" alt="<?= htmlspecialchars($siteName) ?>" title="<?= htmlspecialchars($siteName) ?>" class="register-logo mx-auto">
            <?php endif; ?>
            <h1 class="text-2xl font-bold text-indigo-400">Crear Cuenta</h1>
            <p class="text-sm text-gray-400 mt-1">Regístrate para comprar tus entradas</p>
        </div>

        <?php if ($error): ?>
            <div class="bg-red-600/20 text-red-400 p-3 rounded-lg mb-6 text-sm border border-red-500/30 error-shake" id="errorBox">
        ...
        <form action="" method="POST" class="space-y-4" id="registerForm">
            ...
            <div>
                <label class="block text-sm text-gray-400 mb-1"><i class="fas fa-user mr-1"></i>Nombres y Apellidos *</label>
                <input type="text" name="name" id="name" required value="<?= htmlspecialchars($_POST['name'] ?? '') ?>" maxlength="100" placeholder="Ej: Juan Pérez" class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>

            <div>
                <label class="block text-sm text-gray-400 mb-1"><i class="fas fa-id-card mr-1"></i>Cédula de Identidad *</label>
                <div class="form-group-inline">
                    ...
                    <!-- ✅ CORREGIDO: Sin oninput inline, se maneja en JS -->
                    <input type="text" name="cedula_number" id="cedula_number" required value="<?= htmlspecialchars($_POST['cedula_number'] ?? '') ?>" placeholder="Ej: 1234567" pattern="[0-9]{7,}" minlength="7" maxlength="20" inputmode="numeric" data-only-numbers title="Debe ingresar al menos 7 números" class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>

            <div>
                <label class="block text-sm text-gray-400 mb-1"><i class="fas fa-phone mr-1"></i>Teléfono Móvil *</label>
                <div class="phone-group-inline">
                    ...
                    <!-- ✅ CORREGIDO: Sin oninput inline, se maneja en JS -->
                    <input type="text" name="phone_number" id="phone_number" required value="<?= htmlspecialchars($_POST['phone_number'] ?? '') ?>" placeholder="Ej: 123456" pattern="[0-9]{7,}" minlength="7" maxlength="20" inputmode="numeric" data-only-numbers title="Debe ingresar al menos 7 números" class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>

            <div>
                <label class="block text-sm text-gray-400 mb-1"><i class="fas fa-calendar-alt mr-1"></i>Fecha de Nacimiento *</label>
                <input type="date" name="birth_date" id="birth_date" required value="<?= htmlspecialchars($_POST['birth_date'] ?? '') ?>" max="<?= date('Y-m-d', strtotime('-12 years')) ?>" class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>

            <div>
                <label class="block text-sm text-gray-400 mb-1"><i class="fas fa-envelope mr-1"></i>Correo Electrónico *</label>
                <input type="email" name="email" id="email" required value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" maxlength="100" placeholder="ejemplo@email.com" class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>

            <div>
                <label class="block text-sm text-gray-400 mb-1"><i class="fas fa-lock mr-1"></i>Contraseña *</label>
                <div class="password-wrapper">
                    <input type="password" name="password" id="regPassword" required placeholder="Mínimo 8 caracteres" minlength="8" maxlength="255" pattern=".{8,}" title="La contraseña debe tener al menos 8 caracteres" class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 pr-10">
                    <!-- ✅ CORREGIDO: Sin onclick inline, usa data-attribute -->
                    <button type="button" class="password-toggle" data-toggle-password="regPassword" aria-label="Mostrar contraseña">
            ...
            <div>
                <label class="block text-sm text-gray-400 mb-1"><i class="fas fa-check-circle mr-1"></i>Confirmar Contraseña *</label>
                <div class="password-wrapper">
                    <input type="password" name="confirm_password" id="regConfirmPassword" required placeholder="Repite tu contraseña" minlength="8" maxlength="255" pattern=".{8,}" title="La contraseña debe tener al menos 8 caracteres" class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 pr-10">
                    <!-- ✅ CORREGIDO: Sin onclick inline, usa data-attribute -->
                    <button type="button" class="password-toggle" data-toggle-password="regConfirmPassword" aria-label="Mostrar contraseña">
                        <i class="fas fa-eye"></i>
                    </button>
                </div>
            </div>

            <button type="submit" id="submitBtn" class="w-full bg-indigo-600 hover:bg-indigo-700 p-3 rounded-lg font-bold transition-colors shadow-md mt-2 flex items-center justify-center gap-2">
    ...
    <!-- ✅ CORREGIDO: Script con nonce CSP -->
    <script nonce="<?= htmlspecialchars($cspNonce ?? '') ?>">
    // ============================================
    // TOGGLE PASSWORD VISIBILITY (con event delegation)
    // ============================================
