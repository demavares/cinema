...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($name) || empty($email) || empty($password) || empty($confirm_password) || empty($cedula_number) || empty($phone_number) || empty($birth_date)) {
            ...
            try {
                // ✅ Verificar si la cédula ya se encuentra registrada
                ...
                if ($stmt_check_cedula->rowCount() > 0) {
                    $error = "El usuario con número de cédula " . $cedula_type . '-' . $cedula_number . " ya se encuentra registrado.";
                }
                ...
                if (empty($error)) {
                    ...
                    if ($stmt_check_email->rowCount() > 0) {
                        $error = "El correo electrónico " . htmlspecialchars($email) . " ya está registrado. Por favor, usa otro email.";
                    }
                }
                ...
                if (empty($error)) {
                    ...
                    try {
                        ...
                        $pdo->rollBack();
                        error_log("Error en registro: " . $e->getMessage() . " - Data: " . print_r($_POST, true));
                        
                        if ($e->errorInfo[1] == 1062) {
                            // Duplicado - verificar qué campo causa el duplicado
                            if (strpos($e->getMessage(), 'email') !== false) {
                                $error = "El correo electrónico ya está registrado. Por favor, usa otro.";
                            } elseif (strpos($e->getMessage(), 'cedula') !== false) {
                                $error = "El número de cédula ya está registrado. Por favor, verifica tus datos.";
                            } else {
        ...
        <form action="" method="POST" class="space-y-4" id="registerForm">
            ...
            <div>
                ...
                <div class="form-group-inline">
                    ...
                    <input type="text" name="cedula_number" id="cedula_number" required 
            ...
            <div>
                ...
                <div class="password-wrapper">
                    ...
                    <button type="button" class="password-toggle" onclick="togglePasswordVisibility('regPassword', this)" aria-label="Mostrar contraseña">
            ...
            <div>
                ...
                <div class="password-wrapper">
                    <input type="password" name="confirm_password" id="regConfirmPassword" required 
                           ...
                           maxlength="255"
                           class="w-full bg-gray-700 p-2.5 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 pr-10">
                    <button type="button" class="password-toggle" onclick="togglePasswordVisibility('regConfirmPassword', this)" aria-label="Mostrar contraseña">
                        <i class="fas fa-eye"></i>
                    </button>
                </div>
            </div>
