...
if (isset($_POST['add_movie'])) {
    ...
    // EXTRAER TÍTULO Y AÑO DEL CAMPO TÍTULO
    // Si el título contiene un año al final (Ej: "Superman, 1978")
    $title = $title_raw;
    $extracted_year = null;
    
    // Buscar patrón: ", YYYY" al final del título
    if (preg_match('/,\s*(\d{4})$/', $title_raw, $matches)) {
        $extracted_year = intval($matches[1]);
        // Remover el año del título
        $title = trim(preg_replace('/,\s*\d{4}$/', '', $title_raw));
        // Si no se especificó año en el campo año, usar el extraído
        if (empty($year)) {
            $year = $extracted_year;
        }
    }

    // Director YA NO es obligatorio
    if (empty($title) || empty($trailer_url) || empty($classification)) {
        ...
        // BUSCAR DESDE TMDb USANDO TÍTULO Y AÑO
        ...
        if ($tmdb_data) {
            ...
            // Si no hay datos de TMDb, mostrar error
...
elseif (isset($_POST['edit_movie'])) {
    ...
    // Extraer título y año si viene formateado con coma (Ej: "Superman, 1978")
    $title = $title_raw;
    if (preg_match('/,\s*(\d{4})$/', $title_raw, $matches)) {
        $title = trim(preg_replace('/,\s*\d{4}$/', '', $title_raw));
        if (empty($year)) {
    ...
    if ($id <= 0) {
        ...
        // Guardar directamente en la base de datos los valores ingresados manualmente
        try {
            $stmt = $pdo->prepare("
                UPDATE movies SET 
                    title = ?, 
...
if (isset($_GET['update_movie']) && validateGetAction('update_movie', $_GET['update_movie'])) {
    ...
    try {
        ...
        if (!$movie) {
            ...
            // ✅ Si falla con el año, intentar solo con el título
            if (!$tmdb_data) {
...
// Horarios - Orden ascendente por título de película
...
// ✅ USUARIOS - Búsqueda por cédula (CORREGIDO)
...
// Comida - Orden ascendente por nombre
...
// Obtener formatos únicos de la BD para el select
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" min="0" name="price_child" id="price_child"
                                   ...
                                   class="w-full bg-gray-700 p-2.5 pl-7 rounded text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 price-input-disabled"
                                   placeholder="0.00"
                                   disabled>
                        </div>
...
// ============================================
// ✅ TOGGLE PRECIO (Niño / Tercera Edad)
// ============================================
