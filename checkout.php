...
try {
    ...
    foreach ($seatsArray as $index => $seat) {
        if ($index < $totalAdults) $price = $pricesByType['adult'];
        elseif ($index < $totalAdults + $totalChildren) $price = $pricesByType['child'];
        else $price = $pricesByType['senior'];
    ...
    foreach ($seatsArray as $index => $seat) {
        if ($index < $totalAdults) $ticketTypeId = 1;
        elseif ($index < $totalAdults + $totalChildren) $ticketTypeId = 2;
        else $ticketTypeId = 3;
        
        $price = $index < $totalAdults ? $pricesByType['adult'] : 
                ($index < $totalAdults + $totalChildren ? $pricesByType['child'] : $pricesByType['senior']);
