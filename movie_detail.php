...
// ✅ Limitar el ID a un rango razonable
if ($movie_id > 999999) {
header('Location: index.php?error=id_invalido');
exit;
}
...
// ============================================
// ✅ OBTENER DATOS DE TMDb Y DEFINIR tmdb_id
// ============================================
$tmdb_data = getMovieFromTMDB($movie['title'], $movie['year'] ?? null);
// ✅ DEFINIR tmdb_id (CORRECCIÓN DE BUG: no estaba definida)
$tmdb_id = $tmdb_data['tmdb_id'] ?? null;
// ✅ VALIDAR Y SANITIZAR DATOS DE TMDb ANTES DE USARLOS
if ($tmdb_data) {
// Sanitizar descripción (eliminar etiquetas HTML potencialmente maliciosas)
...
// ✅ Validar que las URLs de imágenes sean de dominios confiables (TMDb)
if (!empty($tmdb_data['poster_path'])) {
if (strpos($tmdb_data['poster_path'], '/') === 0) {
$tmdb_data['poster_path'] = 'https://image.tmdb.org/t/p/w500' . $tmdb_data['poster_path'];
} else {
$tmdb_data['poster_path'] = null;
}
}
if (!empty($tmdb_data['backdrop_path'])) {
if (strpos($tmdb_data['backdrop_path'], '/') === 0) {
$tmdb_data['backdrop_path'] = 'https://image.tmdb.org/t/p/original' . $tmdb_data['backdrop_path'];
} else {
$tmdb_data['backdrop_path'] = null;
}
}
// ✅ Validar que el año sea razonable
if (!empty($tmdb_data['year'])) {
$tmdb_data['year'] = intval($tmdb_data['year']);
if ($tmdb_data['year'] < 1900 || $tmdb_data['year'] > date('Y') + 2) {
$tmdb_data['year'] = null;
}
}
// ✅ Validar que la duración sea razonable
if (!empty($tmdb_data['runtime'])) {
$tmdb_data['runtime'] = intval($tmdb_data['runtime']);
if ($tmdb_data['runtime'] < 1 || $tmdb_data['runtime'] > 720) {
$tmdb_data['runtime'] = null;
}
}
}
...
// ✅ CORREGIDO: Escapar PRIMERO, luego convertir saltos de línea de forma segura
$escaped_description = htmlspecialchars($description, ENT_QUOTES, 'UTF-8');
// Convertir saltos de línea a <br> manualmente (ya está escapado)
