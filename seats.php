...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const isFromFoodMenu = referrer.includes('food_menu.php');
    const isFromPriceSelection = referrer.includes('price_selection.php') || referrer.includes('process_selection.php');

    // ✅ SI VIENE DE PRICE_SELECTION, LIMPIAR ASIENTOS GUARDADOS
    if (isFromPriceSelection) {
        clearSeatsStorage();
        console.log('🗑️ Asientos limpiados (viene de price_selection)');
    }

    // ============================================
    // CARGAR ASIENTOS GUARDADOS (solo si viene de food_menu)
    // ============================================
    let hasSavedSeats = false;
    if (isFromFoodMenu) {
        hasSavedSeats = loadSeatsFromStorage();
        if (hasSavedSeats) {
            console.log('✅ Asientos restaurados (viene de food_menu)');
        }
    }
