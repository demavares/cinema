...
@media (max-width: 480px) {
    ...
    .timeout-btn, .session-expired-btn { padding: 10px 24px; font-size: 0.9rem; width: 100%; }
}
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
        ...
        <div class="movie-card rounded-xl">
            <a href="movie_detail.php?id=<?= intval($movie['id']) ?>" class="poster">
                <img src="<?= htmlspecialchars($poster_url) ?>" alt="<?= htmlspecialchars($movie['title']) ?>" loading="lazy">
                <?php if($hasPresale): ?>
