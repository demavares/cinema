...
// Para cada película, obtener los showtimes para verificar promociones
...
// ============================================
// CARRUSEL - USAR IMÁGENES DE LA BASE DE DATOS
// ============================================
// No obtener datos de TMDb para el carrusel, solo usar lo que está en la DB
        ...
        <section class="hero-carousel">
            <div class="carousel-slides-container" id="carouselSlides">
                <?php foreach ($movies as $index => $m): 
                    ...
                    // CARRUSEL - USAR IMÁGENES DE LA BASE DE DATOS
                    // ============================================
                    // Poster: usar poster_url de la DB
                    $poster_url = $m['poster_url'] ?? '';
                    
                    // Backdrop/Banner: priorizar banner_url de la DB, si no existe usar poster_url
                    $backdrop_url = !empty($m['banner_url']) ? $m['banner_url'] : $poster_url;
                    
                    // Si no hay imagen en la DB, usar un placeholder
                    if (empty($poster_url)) {
                        $poster_url = 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22300%22 height=%22450%22 viewBox=%220 0 300 450%22%3E%3Crect fill=%22%231a1a2e%22 width=%22300%22 height=%22450%22/%3E%3Ctext x=%22150%22 y=%22225%22 text-anchor=%22middle%22 fill=%22%236b7280%22 font-size=%2240%22 font-family=%22Arial%22%3E🎬%3C/text%3E%3C/svg%3E';
                    }
                    ...
                    // Obtener datos para mostrar en el carrusel (desde la DB)
                ...
                ?>
                    <div class="hero-slide">
                        <!-- Fondo: usar banner_url de la DB -->
                        <div class="hero-backdrop" style="background-image: url('<?= htmlspecialchars($backdrop_url) ?>');"></div>
                        
                        <div class="hero-content">
                            <!-- Póster: usar poster_url de la DB -->
                            <a href="movie_detail.php?id=<?= $m_id ?>" class="hero-poster hidden sm:block">
                            ...
                            <div class="hero-info">
                                ...
                                <div class="hero-meta">
                                    ...
                                    <?php if(!empty($m_genre)): ?>
                                        <span class="meta-item">
                                            ...
                                            <?= htmlspecialchars($m_genre) ?>
                                        </span>
                                    <?php endif; ?>
                                    <?php if(!empty($m_classification)): ?>
                                        <span class="meta-item">
                                            <span class="certification-tag b"><?= htmlspecialchars($m_classification) ?></span>
                                        </span>
                                    <?php endif; ?>
                                    <?php if(!empty($m_year)): ?>
                                        <span class="meta-item">
                                ...
                                <?php if(!empty($m_description)): ?>
                                    <p class="overview-text text-sm mb-4">
                                        <?= htmlspecialchars($m_description) ?>
                                    </p>
                                <?php endif; ?>

                                <div class="flex items-center gap-3 flex-wrap">
                                    <a href="movie_detail.php?id=<?= $m_id ?>" class="btn-reserve btn-hero-reserve">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
            ...
            ?>
                <div class="movie-card rounded-xl">
                    <a href="movie_detail.php?id=<?= $movie['id'] ?>" class="poster">
                    ...
                    <div class="p-4">
                        <h3 class="font-bold text-gray-900 text-base truncate" title="<?= htmlspecialchars($movie['title']) ?>">
                            <a href="movie_detail.php?id=<?= $movie['id'] ?>" class="hover:text-indigo-600 transition-colors">
                        ...
                        <div class="info-row">
                            <?php if($certLabel): ?>
                                <span class="certification-tag <?= $certClass ?>"><?= $certLabel ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="flex items-center gap-3 mt-2 text-xs text-gray-500">
                            ...
                            <?php endif; ?>
                            <?php if($movie['year']): ?>
                                <span><i class="far fa-calendar-alt mr-1"></i><?= $movie['year'] ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <button onclick="window.location.href='movie_detail.php?id=<?= $movie['id'] ?>'" 
...
<script>
    ...
    // Limpiar todo el sessionStorage relacionado con comida
    const keysToRemove = [];
    for (let i = 0; i < sessionStorage.length; i++) {
        const key = sessionStorage.key(i);
        if (key && (key.startsWith('food_timeout_') || key.startsWith('food_seats_'))) {
    ...
    function closeTimeoutModal() {
        ...
        // Limpiar el parámetro timeout de la URL sin recargar
